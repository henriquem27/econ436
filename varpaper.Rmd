...
```{r}
ggarrange(
ggAcf(residuals(var1)[,1]),
ggAcf(residuals(var1)[,2]),
ggAcf(residuals(var1)[,3]),
ggAcf(residuals(var1)[,4]))
```
...
```{r}
ggarrange(
ggAcf(residuals(var2)[,1]),
ggAcf(residuals(var2)[,2]),
ggAcf(residuals(var2)[,3]),
ggAcf(residuals(var2)[,4]))
```
...
Looking at the forecasts for period 1 it is pretty clear that the VAR does a lot better in forecasting as it pretty much matches timing the peaks and drops of the data despite not matching the size of the peaks. Meanwhile, in the univariate approach the forecast is doing almost the opposite.
...
For Bitcoin, the VAR outperformed the ARMA forecasts by quite a bit for the first period as we can see by the large difference between the RMSE where the VAR had a 0.05 RMSE and the 
